...
class TADPMethodTester
  ...
  def call(algo)
    devolver_resultado_proc.call(algo.spying_object.lista_metodos_llamados.any? { |tester| tester.call(self.metodo) },
  ...
  def con_argumentos(*args)
    define_singleton_method :call do |espia|
      metodos_que_cumplen_argumentos =
          espia.spying_object.lista_metodos_llamados.select { |tester| tester.call(metodo, args) }
      devolver_resultado_proc.call(metodos_que_cumplen_argumentos!empty?, "\n la funcion esperaba #{args}", args)
    end
    self
  end
end
